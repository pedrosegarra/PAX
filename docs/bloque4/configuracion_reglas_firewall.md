...
• Interfaces
  o in-interface → interfaz por la que entra el tráfico
  o out-interface → interfaz por la que sale el tráfico
• Direcciones IP
  o src-address → dirección o red de origen.
  o dst-address → dirección o red de destino.
• Protocolos y puertos
  o protocol → tcp, udp, icmp, etc.
  o src-port → puerto de origen.
  o dst-port → puerto de destino.
• Estado de la conexión
  o connection-state → new, established, related, invalid.
...
• En el momento en que una regla coincide:
  o Se ejecuta la acción asociada.
  o El procesamiento se detiene para ese paquete.
  o Las reglas posteriores no se evalúan.
...
establecidas (established) o relacionadas (related).
  o established: paquetes que pertenecen a conexiones iniciadas 
  previamente, por ejemplo, respuestas a solicitudes HTTP desde la 
  LAN.
  o related: paquetes asociados a conexiones existentes, como 
